...
- `fields`: only the optional fields that pass their test for *this* term (R-FLD-08 to R-FLD-12). Most patterns, particles, function words and verbs pass the `example` test; the other four fields are the ones that tend to get padded, so each needs a specific reason. There is no target number or ratio (R-FLD-01, R-FLD-16). Apply the tests the same way to the last term as to the first (R-FLD-15). With beginner mode off, scan every term for `controversy` (R-FLD-12).
