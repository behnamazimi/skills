...
2. **Work out what's already known.** Every excluded item counts as known. Mark which building-block jobs (R-SEL-05) it already covers, and which closed sets it has started.
...
7. **Run the shape check** once on the full list (R-SEL-07) and record it in `shape`. Fix failures with the R-SEL-10 cut order, then fill from the next slot. Never add nouns just to look balanced.
8. **Add about 30% spares** (R-SEL-11), drawn from the same slots, next in line.
