...
7. **Map the required jobs** (R-SEL-12, `bare` slices): in `shape.jobs`, give every job for this ceiling an exact `main` term, `known: <excluded item>`, `n/a: <reason>`, or `deferred`. Defer only when every slot is taken, and only from the end of the priority list. A job with no entry means the list is missing a building block: claim a slot for it before any glue, phrase or noun.
